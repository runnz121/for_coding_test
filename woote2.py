def solution(log):
    ...
    for i in range(0, len(times) - 1, 2):
        the_hour = times[i+1][0] - times[i][0]
        if the_hour <= 0:
            the_hour = 0

        print(times[i+1][1], times[i][1])

        if the_hour >= 0 and times[i+1][1] < times[i][1]:
            to_minute = the_hour * 60 - times[i+1][1] - times[i][1]
        ...
        time_spend.append(to_minute)

    total = sum(time_spend)
